...
- **Caps.** NN/g's glanceable-type study found uppercase faster *for a word or two in isolation* — so short column heads and one-word nav group labels may be uppercase at 11–12 px with 0.04–0.06 em tracking. Multi-word strings, statuses read in sequence, window bars with sentences in them, and anything scanned down a column stay sentence case. No monospace at all unless the product's users read code (`SKILL.md` commitment 4): IDs, passwords, env names, placeholders and keyboard hints go in the UI face at 500 with tabular figures (add a copy button where a value is copied). Point the framework's mono token at the UI face so a stray `font-mono` class and the browser's defaults for `code`/`kbd`/`samp`/`pre` follow it.
